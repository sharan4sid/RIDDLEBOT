...
export default function Chatbot(/*{ onConstraintChange }: ChatbotProps*/) { // Remove prop
  const [chatHistory, setChatHistory] = useState<ChatMessage[]>([
    { role: 'bot', content: "Hi there! 👋 What topic would you like for the next riddle? Options: animals, science, history, food, technology, nature, sports, movies, music, geography." },
  ]);
  ...
  const [botState, setBotState] = useState<'topic' | 'difficulty'>('topic');
        ...
        const output: ChatOutput = await chatWithRiddleBot(input);
         ...
         if (output.intent === 'unknown' && (output.response.includes('error') || output.response.includes('busy') || output.response.includes('connect'))) {
            toast({
             ...
             setChatHistory((prev) => [...prev, { role: 'bot', content: output.response }]);
             return;
          }

        let finalResponse = output.response;
        let toastConfig: { title: string, description: string } | null = null;
        let shouldUpdateConstraints = false;
        let constraintString = '';

        if (botState === 'topic') {
          if (output.intent === 'topic' && output.value) {
             setPendingTopic(output.value);
             setBotState('difficulty');
             finalResponse = `Great! I've set the topic to ${output.value}. Now, what difficulty level would you like? (easy, medium, hard)`;
             toastConfig = { title: "Topic Selected", description: `Topic set to: ${output.value}` };
          } else if (output.intent === 'difficulty' && output.value) {
             finalResponse = `I noted the difficulty (${output.value}), but please select a topic first! Options: animals, science, history, etc.`;
          } else {
             if (output.intent !== 'chit_chat') {
                finalResponse = "Please choose a topic first! Options include: animals, science, history, movies, music, etc.";
             }
          }
        } else if (botState === 'difficulty') {
          if (output.intent === 'difficulty' && output.value) {
             constraintString = `topic: ${pendingTopic}, difficulty: ${output.value}`;
             shouldUpdateConstraints = true;
             setBotState('topic'); // reset for next round
             setPendingTopic(null);
             finalResponse = `Perfect! The next riddle will be about ${pendingTopic} and ${output.value} difficulty. Let's play!`;
             toastConfig = { title: "Difficulty Selected", description: `Difficulty set to: ${output.value}` };
          } else if (output.intent === 'topic' && output.value) {
             setPendingTopic(output.value);
             finalResponse = `Okay, I've updated the topic to ${output.value}. Now, what difficulty level would you like? (easy, medium, hard)`;
             toastConfig = { title: "Topic Updated", description: `Topic set to: ${output.value}` };
          } else {
             if (output.intent !== 'chit_chat') {
                finalResponse = `Please choose a difficulty level: easy, medium, or hard.`;
             }
          }
        }

        const botMessage: ChatMessage = { role: 'bot', content: finalResponse };
